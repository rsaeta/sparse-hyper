...
class ClusteredAttention(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.attention = KmeansAttention(
            clusters,
    ...
    def forward(self, x: torch.Tensor, attention_mask: torch.Tensor):
        b, c, e = x.shape
        k = (
            self.to_keys(x).view(b, c, self.num_heads, self.head_size).transpose(1, 2)
        )  # (B, nh, T, hs)
        v = (
            self.to_values(x).view(b, c, self.num_heads, self.head_size).transpose(1, 2)
        )  # (B, nh, T, hs)
        q = (
            self.to_queries(x)
            .view(b, c, self.num_heads, self.head_size)
            .transpose(1, 2)
        )  # (B, nh, T, hs)
        out, loss = self.attention(
            q, k, v
