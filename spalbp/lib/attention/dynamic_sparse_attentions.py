...
class _OneDimensionalSparseAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, attention_mask: torch.Tensor, output_attentions: bool = False) -> torch.Tensor:
        ...
        weights = weights.sum(dim=4)  # I don't get this at all (sum out the MVNs)
        ...
        K = K / (self.head_size ** (1 / 4))
        Q = Q / (self.head_size ** (1 / 4))
        """
        ...
        new_weights = new_weights.log_softmax(dim=-1).exp()  # Normalize attention scores
        ...
        """
        Q = Q / (self.head_size ** (1 / 4))  # Normalize along the embedding dimension
        K = K / (self.head_size ** (1 / 4))
        """
