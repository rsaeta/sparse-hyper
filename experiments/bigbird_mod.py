...
class BigBirdModSelfAttention(nn.Module):
    ...
    def bigbird_block_sparse_attention(
        ...
        # to disable global, just replace first_attn_weights with zeros
        first_attn_weights = torch.zeros_like(first_attn_weights)
        ...
        second_key_mat = torch.cat(
            [
                ...
                torch.zeros_like(blocked_key_matrix[:, :, -1]),  # replacing global block with zeros
                torch.zeros_like(gathered_key[:, :, 0]),  # replacing random block with zeros
            ],
            dim=2,
        )  # [b, h, (4+r)*wn, -1]
        ...
        band_product = torch.cat(
            [torch.zeros_like(first_band_product),
             inner_band_product,
             torch.zeros_like(rand_band_product),
             torch.zeros_like(last_band_product),
            ], dim=-1
        ...
        second_last_value_mat = torch.cat(
            [
                torch.zeros_like(blocked_value_matrix[:, :, 0]),
                blocked_value_matrix[:, :, -3],
                blocked_value_matrix[:, :, -2],
                blocked_value_matrix[:, :, -1],
                torch.zeros_like(gathered_value[:, :, -1]),
            ],
            dim=2,
        )  # [b, h, (4+r)*wn, -1]
        ...
        last_attn_weights = torch.zeros_like(last_attn_weights)
