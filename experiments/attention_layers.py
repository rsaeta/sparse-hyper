...
class DynamicDilatedAttention(nn.Module):
    def __init__(self, stride_predictor: nn.Module, emb: int, k: int = 4, layer: int = 0, **kwargs):
        self.emb = emb
        self.k = k
        self.layer = layer
        ...
        self.register_buffer('mvalues',torch.ones(2*k))

    def hyper(self, x: Tensor) -> Tensor:
        params = self.stride_predictor(self.layer)
        ...
        means = (offsets.expand_as(means) + means)[:,:,None]
        sigmas = sigma[:,None].expand_as(means)
        mvalues = self.mvalues[None, None, :].expand_as(sigmas)
        means = sparse.transform_means(means, (t,), 'clamp')
        sigmas = sparse.transform_sigmas(sigmas, (t,))
        return means, sigmas, mvalues
